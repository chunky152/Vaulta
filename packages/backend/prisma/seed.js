...
const StorageLocation_js_1 = require("../src/models/StorageLocation.js");
const StorageUnit_js_1 = require("../src/models/StorageUnit.js");
const PricingRule_js_1 = require("../src/models/PricingRule.js");
const index_js_1 = require("../src/config/index.js");
async function main() {
    try {
        await mongoose_1.default.connect(index_js_1.config.database.url);
        console.log('Connected to MongoDB');
        console.log('Seeding database...');
        // Create admin user
        const adminPassword = await bcryptjs_1.default.hash('Admin@123456', 12);
        const admin = await User_js_1.User.findOneAndUpdate({ email: 'admin@vaulta.com' }, {
            email: 'admin@vaulta.com',
            passwordHash: adminPassword,
            firstName: 'Admin',
            lastName: 'User',
            role: User_js_1.UserRole.SUPER_ADMIN,
            emailVerified: true,
        ...
        const customer = await User_js_1.User.findOneAndUpdate({ email: 'customer@example.com' }, {
            email: 'customer@example.com',
            passwordHash: customerPassword,
            firstName: 'John',
            lastName: 'Doe',
            phone: '+1234567890',
            role: User_js_1.UserRole.CUSTOMER,
            emailVerified: true,
        ...
        for (const locationData of locations) {
            const location = await StorageLocation_js_1.StorageLocation.findOneAndUpdate({ slug: locationData.slug }, {
                ...locationData,
            ...
            const unitConfigs = [
                { size: StorageUnit_js_1.UnitSize.SMALL, count: 10, priceHourly: 2.5, priceDaily: 15, dimensions: { width: 60, height: 60, depth: 60 } },
                { size: StorageUnit_js_1.UnitSize.MEDIUM, count: 8, priceHourly: 5, priceDaily: 30, dimensions: { width: 90, height: 90, depth: 90 } },
                { size: StorageUnit_js_1.UnitSize.LARGE, count: 5, priceHourly: 8, priceDaily: 50, dimensions: { width: 120, height: 150, depth: 120 } },
                { size: StorageUnit_js_1.UnitSize.XL, count: 3, priceHourly: 15, priceDaily: 80, dimensions: { width: 200, height: 200, depth: 200 } },
            ];
            let unitCounter = 1;
            for (const config of unitConfigs) {
                for (let i = 0; i < config.count; i++) {
                    const unitNumber = `${location.slug.split('-')[1]?.toUpperCase().slice(0, 3) ?? 'UNT'}-${String(unitCounter).padStart(3, '0')}`;
                    await StorageUnit_js_1.StorageUnit.findOneAndUpdate({ locationId: location._id, unitNumber }, {
                        ...
                        currency: 'USD',
                        status: i === 0 ? StorageUnit_js_1.UnitStatus.OCCUPIED : StorageUnit_js_1.UnitStatus.AVAILABLE,
                        features: config.size === StorageUnit_js_1.UnitSize.SMALL
                            ? ['secure']
                            : config.size === StorageUnit_js_1.UnitSize.XL
        ...
        for (const rule of pricingRules) {
            await PricingRule_js_1.PricingRule.findOneAndUpdate({ name: rule.name }, rule, { upsert: true, new: true });
        }
